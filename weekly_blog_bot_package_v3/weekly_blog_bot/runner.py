...
def run(config_path: pathlib.Path, *, dry_run: bool = False, no_calendar: bool = False) -> Dict[str, Any]:
    ctx = stages.stage_prepare(config_path, dry_run=dry_run, no_calendar=no_calendar)
    ...
    if ctx.dry_run:
        ...
        ctx = stages.stage_generate_spec(ctx, client=client)
        ctx = stages.stage_review(ctx, client=client)
        ctx = stages.stage_repair_if_needed(ctx, client=client)
